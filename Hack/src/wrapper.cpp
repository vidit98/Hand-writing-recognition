...
void orient(Mat img, Mat &out)
{
	vector<Vec4i> lines;
	Mat img1;
	Mat vis =  imread("./data/FORM - FREE TEXT INOUT BOXES_1-1.png", 1);
...
void mergeLines(vector<Point> initial_point, vector< vector< Rect> > &grp)
{
	...
	Mat img1 =   imread("oriented.jpg", 1);
...
void detect_word(vector<Rect> boxes, vector<vector<int> > &final_box){
...
vector<vector<vector<int> > > preprocess4(){

	//cout<<"enter preprocess 4 with size" <<final_grp.size()<<endl;
	vector<vector<vector<int> > > ans;
	vector<Rect> line;
	for(int i=0;i<final_grp.size();i++){
		vector<vector<int> > temp;
		line = final_grp[i];
		detect_word(line, temp);
		ans.push_back(temp);
	}
	return ans;
}
